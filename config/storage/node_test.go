...
func TestGetAllService(t *testing.T) {
	Convey("given redis client", t, func() {
		...
		Convey("and 3 services have registered", func() {
			svcKey := func(k string) string { return fmt.Sprintf("%v:%v", SERVICE_ADDRESSES, k) }
			storage.Add(context.Background(), svcKey("sv1"), "123.0.0.1")
			storage.Add(context.Background(), svcKey("sv2"), "123.0.0.2")
			storage.Add(context.Background(), svcKey("sv3"), "123.0.0.3")
